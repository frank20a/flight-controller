...
class AHRSMadgwick(AHRSEstimator):
    def __init__(self, filename, title, gain=0.1):
        super().__init__(filename, f"{title} ($\\beta$= {BETA:.2e}, $\\zeta$= {ZETA:.2e})")
        self.gain = gain
    ...
    def calculate(self):
        ...
        for (i, a), (_, m), (_, g) in zip(
            ...
            q_w = np.array([0, gx, gy, gz], dtype=np.float64) - ZETA * w_b
            q_w_dot = 0.5 * qmul(q, q_w)

            q_dot = q_w_dot - BETA * grad
            q = q + q_dot * self.dt
            q /= np.linalg.norm(q)
