def plot_imu(data, axes, lims = [None, None, None], title_apnds=['', '', '']):
    ...
    ax1.plot(data['ax'], label='Accel X', color='red')
    ax1.plot(data['ay'], label='Accel Y', color='green')
    ax1.plot(data['az'], label='Accel Z', color='blue')
    ax1.plot([9.81, ] * len(data['ax']), label='Gravity (9.81 m/s^2)', color='darkgrey', linestyle='--')
    ax1.plot((data['ax']**2 + data['ay']**2 + data['az']**2)**.5, label='Accel Total', color='purple', linewidth=.5)
    ax1.legend()
